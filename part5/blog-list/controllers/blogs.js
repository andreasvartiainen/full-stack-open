...
blogRouter.get('/', tokenExtractor, userExtractor, async (request, response, next) => {
	const blogs = await Blog.find({}).where('user', request.user.userId).populate('user', { username: 1, name: 1, id: 1 });
	response.json(blogs);
});
